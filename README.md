...
- **@引用**：编辑器右键「插入 @文件引用」，把当前文件/选中代码以
  ...
  项目树右键「插入 @引用」支持文件/目录多选（目录引用不带行号）；
  没有运行中的会话时回退复制到剪贴板。实验性「选区变化自动插入」（默认关）。
- **跑完提醒**：dsh 会话跑完一轮等待输入时发 IDE 通知，点「查看」直达对应
  会话标签（正在查看时不打扰；设置里可关闭）。
- **会话改动清单**：会话右键「查看改动」，从日志提取该会话编辑/新建过的
  文件（edit/write/str_replace_editor），双击在编辑器打开。
- **全局快捷键**：Alt+D 聚焦运行中的会话，没有则新建；终端标签自动命名为
  会话标题（多会话可分辨）。
